...
const ShootingStars = ({
  ...
  useEffect(() => {
    ...
    const createStar = () => {
      ...
      star.style.boxShadow = `0 0 8px 1px ${starColor}, 0 0 15px 3px ${starColor}`;
      
      const trail = document.createElement('div');
      trail.style.position = 'absolute';
      trail.style.width = '100%';
      trail.style.height = '3px';
      trail.style.background = `linear-gradient(90deg, ${starColor}, transparent)`;
    ...
    const animateStar = (star) => {
      ...
      const distance = Math.max(containerRect.width, containerRect.height) * 2;

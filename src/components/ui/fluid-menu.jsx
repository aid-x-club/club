...
export function MenuItem({ children, onClick, disabled = false, icon, isActive = false, title }) {
  return (
    <div
      className={`relative block w-full h-16 text-center cursor-pointer group
    ...
    >
      <span className="flex items-center justify-center h-full mt-[5%]">
        {icon && (
          <span className="h-6 w-6 transition-all duration-200 group-hover:[&_svg]:stroke-[2.5] relative">
            {icon}
            {/* Simple tooltip */}
            {title && (
              <div className="absolute left-full top-1/2 transform -translate-y-1/2 ml-2 px-3 py-1 text-white text-xs font-medium opacity-0 group-hover:opacity-100 transition-opacity duration-300 whitespace-nowrap z-50" style={{minWidth: '80px', textAlign: 'center', textShadow: '1px 1px 2px rgba(0,0,0,0.8)'}}>
                {title}
              </div>
            )}
          </span>
